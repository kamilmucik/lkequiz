    ...
    <KeyboardAvoidingView style={[GlobalStyle.AppContainer, GlobalStyle.AppScreenViewBackgroundColor,{
     ...
     behavior="position">
      <View style={{flex: 1,width: '100%', padding: 5}}>
        ...
        <InputSwitch 
            label="Baza wiedzy" 
            description="Pokaz tylko dobre odpowiedzi" 
            onChange={correctOnly.set} 
            value={correctOnly.value}
            />

        <InputSelect 
            label="Główny dział pytań (3 szybkie) " 
            description="Wybierz" 
            data={MainDepartments}
            onChange={fastQuizDepartment.set} 
            value={fastQuizDepartment.value}
            />
